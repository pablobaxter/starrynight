...
class RecordFieldUtilTest {

    @Test
    fun `handle param test`() {
        val funSpecBuilder = FunSpec.builder("test")

        val lexiconField = RecordField(
            key = "blah",
            record = ObjectField(properties = emptyMap())
        )
        ...
        assertEquals(ClassName("com.foobar", "Foobar"), param.type)
    }
    ...
    fun `handle param test with nullable`() {
        ...
        assertEquals(1, parameters.size)
        val param = parameters.first()
        assertEquals(ClassName("com.foobar", "Foobar").copy(nullable = true), param.type)
    ...
    fun `handle property test`() {
        ...
        assertEquals(ClassName("com.foobar", "Foobar"), property.type)
    }
    ...
    fun `handle property test with nullable`() {
        ...
        val property = properties.first()
        assertEquals("foobar", property.initializer.toString())
        assertEquals(ClassName("com.foobar", "Foobar").copy(nullable = true), property.type)
    }
}
