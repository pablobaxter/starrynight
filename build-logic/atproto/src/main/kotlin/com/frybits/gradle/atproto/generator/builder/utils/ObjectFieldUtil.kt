...
internal fun FunSpec.Builder.handleParam(name: String, lexiconType: ObjectField, isRequired: Boolean, isNullable: Boolean, context: LexiconContext) {
    val typeName = ClassName(context.authority, "${context.name}${name.titleCaseFirstChar()}").copy(nullable = isNullable)
...
internal fun TypeSpec.Builder.handleProperty(name: String, lexiconType: ObjectField, isNullable: Boolean, context: LexiconContext) {
    val typeName = ClassName(context.authority, "${context.name}${name.titleCaseFirstChar()}").copy(nullable = isNullable)
