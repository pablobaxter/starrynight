...
internal fun FunSpec.Builder.handleParam(name: String, lexiconType: BlobField, isRequired: Boolean, isNullable: Boolean) {
    val typeName = TypeNames.Blob.copy(nullable = isNullable)
    ...
    if (!isRequired) {
        if (isNullable) {
            ...
            parameter.defaultValue("%T", TypeNames.EmptyBlob)
        }
    }
...
internal fun TypeSpec.Builder.handleProperty(name: String, lexiconType: BlobField, isNullable: Boolean) {
    val typeName = TypeNames.Blob.copy(nullable = isNullable)
